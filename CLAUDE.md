...
| `batch_fetch_realsee_floorplan.py` | Batch driver for `fetch_realsee_floorplan.py`: read a `scene_name,url` CSV (`data/csv/batch_fetch.csv`), fetch each scene into its own `data/floorplan/<scene_name>/` subfolder via subprocess (full pipeline per scene; one failure never aborts the batch) | `data/floorplan/<scene_name>/{…}` per row | [doc](tools/docs/batch_fetch_realsee_floorplan.md) |
