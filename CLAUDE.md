...
| `fetch_realsee_floorplan.py` | Download the floor plan (户型图): structured `room_layout.json` (per-room names + 3D wall lines) + rendered floor plan / outline PNGs + `house_layout` summary | `data/floorplan/{room_layout.json, images/, floorplan.json}` | [doc](tools/fetch_realsee_floorplan.md) |
